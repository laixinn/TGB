...
class TensorTGNMemory(torch.nn.Module):
    r"""The Temporal Graph Network (TGN) memory model from the
    `"Temporal Graph Networks for Deep Learning on Dynamic Graphs"
    <https://arxiv.org/abs/2006.10637>`_ paper.
    ...
    def __init__(
        ...
        # self.msg_s_store = {}
        # self.msg_d_store = {}
        self.register_buffer("msg_s_store", torch.empty((self.num_nodes, self.raw_msg_dim + 3), device=self.device))
        self.register_buffer("msg_d_store", torch.empty((self.num_nodes, self.raw_msg_dim + 3), device=self.device))
    ...
    def _reset_message_store(self):
        ...
        # self.msg_s_store = self.memory.new_empty((self.num_nodes, self.raw_msg_dim + 3), device=self.device)
        # self.msg_d_store = self.memory.new_empty((self.num_nodes, self.raw_msg_dim + 3), device=self.device)
        self.msg_s_store = self.msg_s_store.fill_(-1)
        self.msg_d_store = self.msg_d_store.fill_(-1)
    ...
    def _get_updated_memory(self, n_id: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        t = torch.cat([t_s, t_d], dim=0)
        aggr = self.aggr_module(msg, self._assoc[idx], t, n_id.size(0))
        # aggr, last_update = self.aggr_module(msg, self._assoc[idx], t, n_id.size(0))
    ...
    def _update_msg_store(
        ...
        # maintain the latest message for each node
        unique_src = src.unique()
        self._assoc[unique_src] = torch.arange(unique_src.size(0), device=unique_src.device)
        _index = self._assoc[src]
        _, argmax = scatter_max(t, _index, dim=0, dim_size=_index.max()+1)
        argmax = argmax[(0 <= argmax) & (argmax < src.size(0))]
        msg_store[src[argmax]] = torch.cat([src[argmax, None], dst[argmax, None], t[argmax, None], raw_msg[argmax]], dim=1)
